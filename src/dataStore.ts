...
class LocalDataStore {
  ...
  private adminLogs: { id: string; time: string; action: string; user: string }[] = [];
    ...
    try {
      ...
      if (storedUser) {
        ...
        this.currentUser = {
          ...
          role: "admin", // Allow full admin access initially
      ...
      // 4. Loading Admin logs
      const storedLogs = localStorage.getItem("voyage_logs");
      if (storedLogs) {
        this.adminLogs = JSON.parse(storedLogs);
      } else {
        this.adminLogs = [
          { id: "log-1", time: new Date(Date.now() - 3600000 * 2).toISOString(), action: "System initialized with luxury presets", user: "System" },
          { id: "log-2", time: new Date(Date.now() - 3600000).toISOString(), action: "Admin dashboard loaded", user: "akhilvarmakshatriya3@gmail.com" }
        ];
        this.saveLogs();
      }
  ...
  public loginUser(email: string, username: string, role: 'user' | 'admin' = 'user') {
    this.currentUser = {
      id: `user-${Date.now()}`,
      email,
      username,
    ...
    this.saveUser();
    this.addLog(`User logged in: ${email}`, username);
  }

  public logoutUser() {
    const prevEmail = this.currentUser?.email || "Guest";
    this.currentUser = null;
    localStorage.removeItem("voyage_user");
    this.addLog(`User logged out`, prevEmail);
  }
  ...
  public updateUserProfile(username: string, bio: string) {
    if (this.currentUser) {
      ...
      this.saveUser();
      this.addLog(`Updated user profile details`, this.currentUser.email);
    }
  }
  ...
  public switchRole() {
    if (this.currentUser) {
      ...
      this.saveUser();
      this.addLog(`Switched security role to ${this.currentUser.role}`, this.currentUser.email);
    }
  }
  ...
  public saveNewTrip(trip: Trip) {
    ...
    this.saveTrips();
    this.addLog(`Created new trip plan to ${trip.destination}`, this.currentUser?.email || "Anonymous");
  }

  public updateTrip(updatedTrip: Trip) {
    this.trips = this.trips.map(t => t.id === updatedTrip.id ? updatedTrip : t);
    this.saveTrips();
    this.addLog(`Updated trip details for ${updatedTrip.destination}`, this.currentUser?.email || "Anonymous");
  }
  ...
  public deleteTrip(tripId: string) {
    ...
    this.saveTrips();
    this.addLog(`Deleted trip plan: ${trip?.destination || tripId}`, this.currentUser?.email || "Anonymous");
  }
  ...
  public duplicateTrip(tripId: string) {
    ...
    if (original) {
      ...
      this.saveTrips();
      this.addLog(`Duplicated trip ${original.destination}`, this.currentUser?.email || "Anonymous");
    }
  }
  ...
  public toggleFavorite(tripId: string) {
    this.trips = this.trips.map(t => {
      if (t.id === tripId) {
        ...
        this.addLog(`Toggled favorite on ${t.destination}: ${nextFav}`, this.currentUser?.email || "Anonymous");
  ...
  public addExpense(tripId: string, expense: Expense) {
    ...
    this.saveTrips();
    this.addLog(`Added expense '${expense.title}' of ${expense.amount} USD`, this.currentUser?.email || "Anonymous");
  }
  ...
  public deleteExpense(tripId: string, expenseId: string) {
    ...
    this.saveTrips();
    this.addLog(`Deleted expense with ID ${expenseId}`, this.currentUser?.email || "Anonymous");
  }
  ...
  private addLog(action: string, user: string) {
    const log = {
      id: `log-${Date.now()}`,
    ...
    this.adminLogs = [log, ...this.adminLogs].slice(0, 50); // Keep last 50
    this.saveLogs();
  }
  ...
  private saveLogs() {
    localStorage.setItem("voyage_logs", JSON.stringify(this.adminLogs));
  }
  ...
  public addCustomLog(message: string, category: string, user: string) {
    const log = {
      id: `log-${Date.now()}`,
      time: new Date().toISOString(),
      action: `[${category.toUpperCase()}] ${message}`,
      user
    };
    this.adminLogs = [log, ...this.adminLogs].slice(0, 50);
    this.saveLogs();
  }
...
export function addAdminLog(message: string, category: string, user: string) {
  dataStore.addCustomLog(message, category, user);
  return dataStore.getLogs();
}
